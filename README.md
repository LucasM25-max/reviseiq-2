...
creation, onboarding, and the full Biology navigation structure with the
design system in place. Revision content (notes, flashcards, questions,
practicals detail) is intentionally left blank — see "What's not built yet"
below.
...
- **Biology hub** (`/subjects/biology`): all 7 AQA topics across both
  papers, linking into 8 study modules (notes, flashcards, question bank,
  required practicals, six-mark builder, maths skills, command words,
  timetable) — all present with working navigation and empty states, no
  content yet
- Full design system: colour tokens, type scale (Bricolage Grotesque /
  ...
  used for both the exam countdown and topic mastery, dark-mode-ready CSS
  variables, visible focus states, `prefers-reduced-motion` support
...
- Real authentication — `lib/store.tsx` explains this in detail. Usernames
  and schools persist in `localStorage`; passwords are validated on the form
  but never stored anywhere. This needs a real backend (hashed credentials,
  a database, sessions) before any real student data touches it — e.g.
  NextAuth, Supabase Auth, or a custom API.
...
- Dyslexia-friendly font toggle and colour-blind-safe topic palette
  (mentioned in the product spec, not yet implemented)
...
app/                    Routes (App Router)
  ...
  dashboard/
  subjects/biology/      Hub + 8 module pages
...
lib/
  ...
  data/biologyTopics.ts   AQA Biology spec structure (topics, modules,
                           required practicals) -- structural only, no content
```
